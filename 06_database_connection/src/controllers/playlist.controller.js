...
const getPlaylistById = asyncHandler(async (req, res) => {
    ...
    const playlistVideos = await Playlist.aggregate([
        ...
        {
            $match: {
                "videos.isPublished": true // only get the published videos
            }
        },
        {
            $lookup: {
...
const addVideoToPlaylist = asyncHandler(async (req, res) => {
    ...
    const updatedPlaylist = await Playlist.findByIdAndUpdate(
        playlist?._id, // this will be the playlist id where we want to add the video
        {
            $addToSet: {
                videos: videoId,
            },
        },
        { new: true } // new: true will return the updated playlist
    );
...
const removeVideoFromPlaylist = asyncHandler(async (req, res) => {
    ...
    const updatedPlaylist = await Playlist.findByIdAndUpdate(
        playlistId,
        {
            $pull: {
                videos: videoId,
            }, // pull method will remove the videoId from the videos array
        },
        { new: true }
    );
